...
class PaymentCommandMapperTest {
	...
	void convertCompoundPaymentDtoToPaymentCommands() {
		// given
		BookingRequest.Payment payment = new BookingRequest.Payment(
			100000L,
			List.of(
				new BookingRequest.Payment.Method(PaymentType.CREDIT_CARD, 80000L, "tok_card_test_123", null),
		...
		assertThat(((PaymentDetail.CardPaymentDetail) cardCommand.paymentDetail()).cardToken()).isEqualTo("tok_card_test_123");
	...
	void creditCardPaymentWithoutTokenThrowsException() {
		...
		BookingRequest.Payment payment = new BookingRequest.Payment(
			50000L,
			List.of(
				new BookingRequest.Payment.Method(PaymentType.CREDIT_CARD, 50000L, null, null) // cardToken 누락
			)
		);

		// when & then
		assertThatThrownBy(() -> mapper.toCommands(payment, 1001L, "key-idempotency-002"))
			.isInstanceOf(IllegalArgumentException.class)
			.hasMessageContaining("cardToken은 필수입니다.");
	}
}
